...
~/gam-ngs/bin/gam-create --master-bam ~/scripts/master_tcas_4.0_contigs_PE_bams.txt --slave-bam ~/scripts/slave_scaffolds_PE_bams.txt --min-block-size 10 --output ./gam-ngs_merge2/master.tcas4.0.slave.scaffolds > ${SCRIPT_PATH}/gam-create.log.out 2> ${SCRIPT_PATH}/gam-create.log.err
# GAM-NGS: Merge

echo -e "### GAM-NGS's merging phase\n"
echo -e "gam-merge --blocks-file ${SCRIPT_PATH}/gam-ngs_merge2/out.blocks --master-bam ${SCRIPT_PATH}/gam-ngs_merge2/Allpaths-LG.PE.list.txt --master-fasta ${SCRIPT_PATH}/Assembly/Allpaths-LG/genome.ctg.fasta --slave-bam ${SCRIPT_PATH}/gam-ngs_merge2/MSR-CA.PE.list.txt --slave-fasta ${SCRIPT_PATH}/Assembly/MSR-CA/genome.ctg.fasta --min-block-size 10 --output ${SCRIPT_PATH}/gam-ngs_merge2/out --threads ${THREADS_NUM} >${SCRIPT_PATH}/gam-merge.log.out 2>${SCRIPT_PATH}/gam-merge.log.err\n"
